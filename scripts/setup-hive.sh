...
function installHive {
	...
	ln -s /usr/local/apache-$HIVE_VERSION-bin $HIVE_PREFIX
	mkdir $HIVE_PREFIX/logs $HIVE_PREFIX/derby/
}
...
function runHiveServices {
    ...
    nohup hive --service metastore < /dev/null > $HIVE_PREFIX/logs/hive_metastore_`date +"%Y%m%d%H%M%S"`.log 2>&1 &

	echo "running hive server2"
    nohup hive --service hiveserver2 < /dev/null > $HIVE_PREFIX/logs/hive_server2_`date +"%Y%m%d%H%M%S"`.log 2>&1 &
}
